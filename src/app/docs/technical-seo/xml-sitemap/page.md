...
{% recommendation %}
Грешен формат на XML карта. В момента https://guns.bg/sitemap.xml е HTML страница и не е в подходящия XML формат (Incorrect http header content-type: "text/html; charset=utf-8" (expected: "application/xml")). По тази причина, Google Search Console я отхвърля. Препоръчително е да изработите индекс от карти и отделни карти, които да отговарят на описаните XML изисквания.
...
Всички XML карти трябва да съдържат само ресурси, които връщат статус код 200. Отделните карти трябва да бъдат откриваеми на посочените по-горе адреси. Отделните карти в индекса трябва да са ограничени до 50 мб големина и 50 000 адреса в тях.

Трбява да обедините всички карти в индекса, който имате в момента: /sitemap-index.xml.
{% /recommendation %}
...
{% recommendation %}
XML карти трябва да се генерират динамично веднъж дневно, независимо от заявките към тях.
{% /recommendation %}
...
{% recommendation %}
URL адреси с код 404, които са включени в sitemap. Трябва да се прамахнат. В sitemap трябва да има само ресурси с код 200, които да са indexable
{% /recommendation %}
...
{% recommendation %}
Страници затворени за индексация в XML Sitemaps. Препоръчително е да ги премахнете или поправите. В XML карти е позволено само адреси, които връщат статус код 200.
{% /recommendation %}
...
{% recommendation %}
URL адреси, които не са реферирани в нито един XML Sitemap, но които дават код 200 и сa indexable. Препоръчително е да ги добавите към правилния.
{% /recommendation %}
...
{% recommendation %}
В XML сайтмаповете да се добави <lastmod> на всяка страница по сайтмаповете с дата и час на последната промяна по страницата.
...
- [Това е точния синтаксис, който трябва да се спазва, единствено часовата зона зависи от настройката на сървъра](Това е точния синтаксис, който трябва да се спазва, единствено часовата зона зависи от настройката на сървъра)
...
Препоръчително е да се използват някои от таргет ключовите думи. Препоръчително е да се маркират с Schema markup.
{% /recommendation %}
